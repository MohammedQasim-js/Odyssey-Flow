...
export const LiveQueueView: React.FC<LiveQueueViewProps> = ({
  ...
  const [filter, setFilter] = useState<
    "all" | "waiting" | "stagnant" | "skipped"
  >("all");
  const [pulseFilter, setPulseFilter] = useState<string>("all");
  const [clinicalSignalFilter, setClinicalSignalFilter] =
    useState<string>("all");
  ...
  const getPulseInfo = (t: Token) => {
    const doc = doctors.find(
      (d) => d.id === t.doctorId || d.name === t.doctorName,
    );
    const pulse = SanctuaryPulseEngine.computePulseState(
      t,
      doc,
      t.queuePosition,
      t.sanctuary_state as any,
    );
    return {
      title: pulse.title,
  ...
  const getClinicalSignal = (t: Token) => {
    ...
    if (t.stagnationFlag || t.dpsScore <= 65) {
      return {
        ...
        classes:
          "bg-amber-50 text-amber-900 border-amber-300 font-bold hover:bg-amber-100/80",
        isAlert: true,
      };
    }
    if (t.dpsScore >= 85) {
  ...
  const filteredQueue = queue.filter((t) => {
    if (filter === "waiting" && t.status !== "waiting" && t.status !== "called")
      return false;
    ...
    if (pulseFilter !== "all") {
      ...
      if (pulseFilter === "stable" && pulse.code !== "QUEUE_STABLE")
        return false;
      if (pulseFilter === "moving" && pulse.code !== "QUEUE_MOVING")
        return false;
      if (pulseFilter === "reviewing" && pulse.code !== "DOCTOR_REVIEWING")
        return false;
      if (pulseFilter === "almost_turn" && pulse.code !== "ALMOST_YOUR_TURN")
        return false;
      if (pulseFilter === "delayed" && pulse.code !== "SLIGHT_DELAY")
        return false;
      if (
        pulseFilter === "your_turn" &&
        pulse.code !== "YOUR_TURN" &&
        pulse.code !== "PLEASE_REPORT"
      )
        return false;
    }
  ...
  const stagnantCount = queue.filter(
    (t) => t.stagnationFlag && t.status === "waiting",
  ).length;
  const stableCount = queue.filter(
    (t) => !t.stagnationFlag && t.status !== "skipped" && t.dpsScore >= 70,
  ).length;
  const attentionCount = queue.filter(
    (t) =>
      (t.stagnationFlag ||
        (t.dpsScore >= 50 && t.dpsScore < 70) ||
        t.sanctuary_state === "SLIGHT_DELAY") &&
      t.status !== "skipped",
  ).length;
  const urgentCount = queue.filter(
    (t) =>
      t.status === "skipped" ||
      t.priority === "urgent" ||
      (t.dpsScore < 50 && t.stagnationFlag),
  ).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            >
              {isPaused
                ? "⏸ Paused"
                : `${queue.filter((t) => t.status === "waiting").length} Active in Lounge`}
            </span>
          </div>
          <p className="text-xs text-stone-500 mt-1">
            Zero-friction token routing powered by Longitudinal Diagnostic
            Progress Score (DPS).
          </p>
        </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-amber-900">
                {stagnantCount} Patient with Diagnostic Stagnation Detected in
                Queue (e.g. Token A-27)
              </h4>
              <p className="text-[11px] text-amber-800 mt-0.5">
                Patients with repeat scans and multi-specialty transitions are
                prioritized for senior clinical review.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-3xl p-5 border border-stone-200/80 shadow-2xs">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-extrabold text-stone-800 uppercase tracking-wider">
            Active Clinician Micro-Statuses (Controls Sanctuary Pulse on Patient
            Mobile)
          </span>
            ...
            >
              <div>
                ...
                <span className="text-[11px] text-stone-500">
                  Room {doc.room} • {doc.specialty.split(" ")[0]}
                </span>
              </div>
              <select
                value={doc.status}
                onChange={(e) =>
                  handleDoctorStatusChange(
                    doc.id,
                    e.target.value as Doctor["status"],
                  )
                }
      ...
      <div className="bg-white rounded-3xl border border-stone-200/80 shadow-2xs overflow-hidden">
        ...
        {filteredQueue.length === 0 ? (
          <div className="p-10">
            <EmptyState
              type={
                filter === "stagnant" ? "no-diagnostic-concern" : "no-queue"
              }
              title={
                filter === "stagnant"
            ...
            <table className="w-full min-w-[1180px] text-left text-xs text-stone-700">
              <thead className="bg-stone-50/70 border-b border-stone-200/80 text-[10px] font-extrabold text-stone-400 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="whitespace-nowrap py-4 px-3 hidden md:table-cell">
                    Reason
                  </th>
                  <th className="whitespace-nowrap py-4 px-3">Doctor</th>
                  <th className="whitespace-nowrap py-4 px-3 hidden lg:table-cell">
                    Queue Pos
                  </th>
                  <th className="whitespace-nowrap py-4 px-3">Status</th>
                  <th className="whitespace-nowrap py-4 px-3">
                    Sanctuary Pulse
                  </th>
                  <th className="whitespace-nowrap py-4 px-3">
                    Clinical Signal
                  </th>
                  <th className="whitespace-nowrap py-4 px-4 pr-6 text-right">
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {filteredQueue.map((token) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 2. Token */}
                      <td
                        className="py-5 px-3 whitespace-nowrap"
                        style={{ minWidth: 76 }}
                      >
                        <span
                      ...
                      <td className="py-5 px-3">
                        {signal.key !== "no_signal" ? (
                          <button
                            ...
                            title={
                              signal.isAlert
                                ? "Click to open Diagnostic Journey"
                                : undefined
                            }
                          >
                            {signal.isAlert && (
                              <AlertTriangle className="w-2.5 h-2.5 shrink-0 text-amber-700" />
                            )}
                            <span>{signal.label}</span>
                          </button>
                        ) : (
                          <span className="text-stone-400 font-mono pl-3">
                            —
                          </span>
                        )}
                      </td>
      ...
      <PatientQuickViewDrawer
        ...
        doctor={doctors.find(
          (d) =>
            d.id === quickViewToken?.doctorId ||
            d.name === quickViewToken?.doctorName,
        )}
