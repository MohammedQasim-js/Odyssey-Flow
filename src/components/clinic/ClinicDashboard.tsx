...
export const ClinicDashboard: React.FC<ClinicDashboardProps> = ({
  ...
  const [patientFlowFilter, setPatientFlowFilter] = useState<
    "all" | "waiting" | "in_treatment" | "stagnant" | "completed"
  >("all");
  ...
  const inConsultTokens = queue.filter(
    (t) => t.status === "in_consultation" || t.status === "called",
  );
  const completedTokens = queue.filter((t) => t.status === "completed");
  const skippedTokens = queue.filter((t) => t.status === "skipped");
  const stagnantTokens = queue.filter(
    (t) => t.stagnationFlag && t.status !== "completed",
  );

  const currentServingToken =
    inConsultTokens[0] || queue.find((t) => t.queuePosition === 0);
  ...
  const aaravToken =
    queue.find(
      (t) =>
        t.tokenNumber === "A-27" ||
        (t.patientName && t.patientName.includes("Aarav")),
    ) || queue[0];
  ...
  const filteredQueue = useMemo(() => {
    return queue.filter((t) => {
      ...
      if (patientSearch.trim()) {
        ...
        const matchSymptom = (t.symptomsSummary || "")
          .toLowerCase()
          .includes(query);
        if (!matchName && !matchToken && !matchDoctor && !matchSymptom)
          return false;
      }

      // Status filter
      if (patientFlowFilter === "waiting")
        return t.status === "waiting" || t.status === "called";
      if (patientFlowFilter === "in_treatment")
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between px-1">
          ...
          >
            {showDemoControls
              ? "Collapse Simulation Bar ▲"
              : "Expand Simulation Bar ▼"}
          </button>
        </div>

        {showDemoControls && (
          <DemoControlCenter queue={queue} onStateChange={onStateChange} />
        )}
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-2xs flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-1">
            Odyssey Flow Command Center • Suites 101–106 • Real-time queue
            synthesized with Longitudinal Diagnostic Progress analysis.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          >
            ...
            <span>
              Call Next ({nextUpToken ? nextUpToken.tokenNumber : "A-27"})
            </span>
          </button>
        </div>
      </div>
      ...
      {aaravToken && aaravToken.stagnationFlag && (
        <div className="p-5 rounded-3xl bg-amber-50/80 border border-amber-300/80 shadow-2xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in fade-in duration-200">
            ...
            <div>
              <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                ...
                <span className="text-xs font-black text-stone-900">
                  Token {aaravToken.tokenNumber} ({aaravToken.patientName}) —
                  Diagnostic Stagnation Detected
                </span>
              ...
              <p className="text-xs text-amber-950/80 mt-1 leading-relaxed max-w-3xl">
                3 prior uncoordinated outpatient encounters across Cardiology
                &amp; Pulmonology for persistent chest tightness without
                etiology confirmation. Recommended 15-min consultation buffer
                &amp; immediate multidisciplinary clinical snapshot review.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 shrink-0 w-full md:w-auto">
            ...
            <button
              onClick={() =>
                handleDoctorReviewHistory(aaravToken.id, aaravToken.doctorId)
              }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white border border-stone-200/80 shadow-2xs relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase font-extrabold text-stone-400 tracking-wider">
              Active Queue
            </span>
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-black text-stone-900">
              {waitingTokens.length + inConsultTokens.length}
            </span>
          ...
          <div className="mt-2 text-[11px] text-stone-500 flex items-center justify-between">
            ...
            <span className="font-semibold text-emerald-700">
              {inConsultTokens.length} in consult
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stone-100">
            <div>
              ...
              <p className="text-xs text-stone-500 mt-0.5">
                Real-time queue synchronized with diagnostic progress and
                clinical telemetry.
              </p>
            </div>
            ...
            <table className="w-full min-w-[1040px] text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-stone-200 text-stone-400 font-extrabold uppercase text-[10px] tracking-wider">
                  ...
                  <th className="whitespace-nowrap pb-4 pr-2 text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {filteredQueue.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Reason */}
                      <td
                        className="py-5 text-stone-600 max-w-[130px] truncate"
                        title={t.symptomsSummary}
                      >
                      ...
                      <td className="py-5 text-stone-800 font-medium">
                        ...
                        <span className="text-[10px] text-stone-400 font-mono">
                          {t.roomNumber || "Room 101"}
                        </span>
                      </td>

                      {/* Token */}
                      <td
                        className="py-5 font-mono font-black text-stone-900 whitespace-nowrap"
                        style={{ minWidth: 76 }}
                      >
                        <span
                      ...
                      <td className="py-5 pr-2 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-1">
                          ...
                          {isWaiting && (
                            <button
                              onClick={() =>
                                handleDoctorReviewHistory(t.id, t.doctorId)
                              }
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-2xs space-y-4">
            ...
            <p className="text-xs text-stone-500 leading-relaxed">
              Signature empathetic waiting engine. Broadcasts transparent
              clinical context to eliminate patient waiting anxiety.
            </p>
            ...
            <div className="grid grid-cols-2 gap-2.5 text-xs">
              <div className="p-3 rounded-2xl bg-emerald-50/80 border border-emerald-200/80">
                ...
                <strong className="text-sm font-black text-emerald-950 block mt-0.5">
                  Clinic Stable
                </strong>
              ...
              <div className="p-3 rounded-2xl bg-stone-50 border border-stone-200/80">
                ...
                <strong className="text-sm font-black text-stone-900 block mt-0.5">
                  12 min avg
                </strong>
            ...
            <div className="p-3 rounded-2xl bg-stone-900 text-white text-xs">
              ...
              <p className="text-[11px] text-stone-300 mt-1 leading-relaxed">
                By contextualizing why delays happen (e.g. "Doctor reviewing
                history"), patient perceived wait stress is reduced by ~40%.
              </p>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="h-3 w-full bg-stone-100 rounded-full overflow-hidden flex">
                <div
                  style={{
                    width: `${(inConsultTokens.length / Math.max(queue.length, 1)) * 100}%`,
                  }}
                ...
                <div
                  style={{
                    width: `${(waitingTokens.length / Math.max(queue.length, 1)) * 100}%`,
                  }}
                ...
                <div
                  style={{
                    width: `${(stagnantTokens.length / Math.max(queue.length, 1)) * 100}%`,
                  }}
                ...
                <div
                  style={{
                    width: `${(completedTokens.length / Math.max(queue.length, 1)) * 100}%`,
                  }}
              ...
              <div className="h-14 flex items-end justify-between gap-1.5 pt-2">
                {[
                  ...
                  >
                    ...
                    />
                    <span
                      className={`text-[9px] font-mono ${bar.active ? "font-bold text-stone-900" : "text-stone-400"}`}
                    >
                      {bar.time.split(" ")[0]}
                    </span>
                  </div>
      ...
      <div className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-2xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-stone-100">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Identifies patients experiencing diagnostic stagnation,
              uncoordinated multi-specialty consultations, or repeated
              investigations.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs font-medium text-amber-950">
                  Chief Concern: Persistent chest tightness &amp; dyspnea on
                  exertion.
                </p>
              </div>
            ...
            <div className="p-3 rounded-2xl bg-white/80 border border-amber-200/80 text-xs space-y-1.5">
              <div className="flex items-center justify-between text-[11px]">
                <span className="font-bold text-stone-700">
                  3 Prior Uncoordinated Encounters
                </span>
              ...
              <p className="text-[11px] text-stone-600 leading-relaxed">
                2 prior normal ECGs, 2 redundant chest X-rays. No
                cross-specialty reconciliation recorded between departments.
              </p>
              <div className="text-[10px] font-medium text-emerald-800 flex items-center space-x-1 pt-1">
                <CheckCircle2 className="w-3 h-3 text-emerald-600 shrink-0" />
                <span>
                  Recommendation: 15m consultation buffer &amp; Pre-consultation
                  clinical snapshot review.
                </span>
              </div>
            </div>
          ...
          <div className="p-5 rounded-3xl bg-stone-50 border border-stone-200/80 space-y-3 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs font-medium text-stone-700">
                  Chief Concern: 4th dermatology revisit for recurring facial
            ...
            <div className="p-3 rounded-2xl bg-white border border-stone-200/80 text-xs space-y-1.5">
              <div className="flex items-center justify-between text-[11px]">
                <span className="font-bold text-stone-700">
                  Multi-Episode Topicals Ineffective
                </span>
                <span className="text-stone-500">Dermatology Suite 104</span>
              </div>
              <p className="text-[11px] text-stone-600 leading-relaxed">
                Repeated topical corticosteroids prescribed without antinuclear
                antibody (ANA) autoimmune screening.
              </p>
              <div className="text-[10px] font-medium text-emerald-800 flex items-center space-x-1 pt-1">
                <CheckCircle2 className="w-3 h-3 text-emerald-600 shrink-0" />
                <span>
                  Recommendation: Suggest Rheumatology cross-consultation.
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 pt-1">
              <button
                onClick={() => {
                  const target =
                    queue.find((t) => t.tokenNumber === "C-08") || aaravToken;
