...
import { 
  Home, HeartPulse, PlusCircle, Clock, Receipt, 
  ChevronLeft, Sparkles, Smartphone, Maximize2, Minimize2,
  Building2, ShieldCheck, User, Calendar, Activity,
  Wifi, Battery, Signal, ArrowLeft
} from 'lucide-react';
...
export const PatientShell: React.FC<PatientShellProps> = ({ 
  ...
  const [viewportWidth, setViewportWidth] = useState<'375' | '390' | '412' | 'full'>('390');
  
  const [activeToken, setActiveToken] = useState<Token | undefined>(DataStore.getActiveToken());
  const [selectedClinic, setSelectedClinic] = useState<Clinic>(() => DataStore.getClinics()[0]);
  ...
  const handleStartBooking = (clinic: Clinic, doctor?: Doctor) => {
    setSelectedClinic(clinic);
    setSelectedDoctor(doctor || DataStore.getDoctors().find(d => d.clinicId === clinic.id));
  ...
  const getViewportWidthClass = () => {
    switch (viewportWidth) {
      case '375':
        return 'w-[375px] max-w-[375px] min-h-[812px]';
      case '390':
        return 'w-[390px] max-w-[390px] min-h-[844px]';
      case '412':
        return 'w-[412px] max-w-[412px] min-h-[860px]';
      case 'full':
        return 'w-full max-w-xl min-h-[780px]';
      default:
        return 'w-[390px] max-w-[390px] min-h-[844px]';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F6] py-3 sm:py-6 px-2 flex flex-col items-center justify-center font-sans text-stone-900">
      ...
      <div
        className={`bg-[#FAF9F6] transition-all duration-300 relative shadow-[0_20px_60px_rgba(0,0,0,0.12)] border-[9px] border-stone-900 rounded-[3rem] overflow-hidden flex flex-col ${getViewportWidthClass()}`}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-stone-200/80 px-3 py-2 flex items-center justify-around z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all ${
              activeTab === 'home' || activeTab === 'clinics' || activeTab === 'clinic_detail' || activeTab === 'booking' || activeTab === 'review_pay'
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all relative ${
              activeTab === 'pulse' ? 'text-[#16A34A] font-extrabold' : 'text-stone-400 hover:text-stone-700'
            }`}
          >
            <div className={`p-1 rounded-xl relative transition-colors ${
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all ${
              activeTab === 'history' ? 'text-[#16A34A] font-extrabold' : 'text-stone-400 hover:text-stone-700'
            }`}
          >
            <div className={`p-1 rounded-xl transition-colors ${
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all ${
              activeTab === 'appointments' ? 'text-[#16A34A] font-extrabold' : 'text-stone-400 hover:text-stone-700'
            }`}
          >
            <div className={`p-1 rounded-xl transition-colors ${
              activeTab === 'appointments' ? 'bg-emerald-50 text-[#34C759]' : ''
            }`}>
              <Calendar className="w-5 h-5" />
            </div>
            <span className="text-[10px] mt-0.5 tracking-tight">Appointments</span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all ${
              activeTab === 'profile' || activeTab === 'billing' ? 'text-[#16A34A] font-extrabold' : 'text-stone-400 hover:text-stone-700'
            }`}
          >
            <div className={`p-1 rounded-xl transition-colors ${
